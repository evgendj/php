...
ТИПЫ ПЕРЕМЕННЫХ 125874365
ОПЕРАЦИИ с переменными 548215598
КОНСТАНТА ****7145236
if-else *1100258
break - continue  -2569845723
ДЕСТРУКТОР  - 893245
КОНСТРУКТОР - 576987 - и другие магические методы
МАССИВ    --- *5864775
ООП       --- *5988
...
GIT  *********34569871
МАТЕМАТИКА  **9563214
СЕРИАЛИЗАЦИЯ  595232388

**************************  PHP  ***********************************************
// Появился в 1998 году, как более удобный вариант работы в интернете, чем язык Perl, и Perl был основой языка. Версия PHP3 и выше на языке C, точнее используется движок Zend Engine построеный на языке C.
// Facebook и VK, сервисы TED, YouTube, BlaBlaCar, Wikipedia - написаны на PHP. Wordprees тоже на нем, Битрикс.
// PHP расшифровывается как Hypertext Preprocessor – препроцессор гипертекста. (но первичное название Personal Home Page Tools (Инструменты для создания персональных веб-страниц)).
// Язык сценарный, т.е. для запуска требует интерпретатор.
// Прост в изучении. PHP неразрывно связаны с HTML и они используются в одном коде совместно. Но расширение должно быть .php.

**************************  ПЕРЕМЕННЫЕ  ****************************************
// Переменная - именованная выделенная области памяти, содержащей заданный тип и значение, и привязка к блоку памяти.
$a; // Инициализация - объявление переменной.
$a = $b = 50; // Инициализация нескольких переменных и присвоение им значения.
...
--------------------------  ТИПЫ  ------125874365-------------------------------
  // Не строго типизированный язык PHP логически понимает переменные которые вводятся. Но тип можно указывать явно.
int, integer, long // целое число (целочисленный тип). Выяснить максимальный размер допустимого числа можно по функции
  ...
  // для 32-разрядной от –2 147 483 648 до 2 147 483 647, 64-разрядной от –9 223 372 036 854 775 807 до 9 223 372 036 854 775 807
...
NAN // недопустимое число. Константа - любые операции с ней возвращают NAN.
  // echo sqrt(-1); - выведет NAN, те выход за недопустимое число
  // А если делить на 0, будет бесконечность
string // Строковый тип - строка текста, используется для любых данных. Строка ограницена 2 Гб. Но php скрипт ограничен в объеме потребляемой памяти директивой limit (по умолчанию 128 мб)
  // '10' - считается строкой, но можно проводить математические действия. Но после мат. действий уже превращается в число.
array // Ассоциативный массив (хэш масив, хэш набор, ХЭШ). Определяется наличием ключ=>значение. Числовой масив тоже по сути ассоциативный.
list // Я так понял этот тип годится для присвоения, не экспериментировал. Это тот числовой массив.
object // ссылка на объект.
resource // ресурс, может быть ссылки на что-то
...
--------------------------  ОПЕРАЦИИ, СИНТАКСИС  -----548215598-----------------
<?php // Это тэги в которые нужно оборачивать PHP ?> // <?= Такой вариант тоже допустим, но лучше не используй, он только внутри HTML использоваться может.
// Комментарий однострочный
/*
Коммент многострочный
*/
# Комментарий в стили оболочки UNIX (знак шарп)
// Часто в конце строки кода ставится точка с запятой - т.е. конец команды, но не всегда!
...
== // Равно
=== // Эквивалентно
|| // Логическое ИЛИ. or - отличается более низким приоритетом, можно в скобки не заключать - останавливается при значении тру
&& // Логическое И. and - отличается более низким приоритетом, можно в скобки не заключать (при арифметических действиях). При логических действиях оператор функция останавливается при значении фолс
...
<<<MARKER любое слово // here-документ, встраиваемые документы - Когда нужно вывести большой блок на экран. Ставится <<<MENU - три знака меньше и ограничитель, ниже код, затем еще раз ограничитель. В конце ограничитель только с нов. строки, может быть ; если нужно закончить функцию. Тип пишет, лучше от них отказаться. Если закавычить первый ограничитель (только ограничитель), то переменные не будет выводить - NOW-документ (актуальный документ).
  echo <<<MARKER // никаких пробельных символов тут
  Тут большой текст, тэги, // вместо переноса выводит пробел и текст одной строкой
  с переносами чрез энтер
  MARKER; // тоже никаких символов
<<<'MARKER'// переменные текст будет выводить как есть, только имена.
...
**************************  КОНСТАНТА  ******7145236****************************
// Принято писать заглавными. Устанавливается один раз, переопределить нельзя. Выводится только конкотенацией, потому что это не переменная, перед которой есть доллар. Имя не должно повторяться.
define($a, $b) // Определение константы с именем в а и значением в б. Если после б в скобках после запятой дописать $case_sen = false - регистр не будет учитываться, но вроде отменили этот атрибут.
  define("PI", 3.14); // Кавычки обязательны
  echo PI;
// В основном имя константы не будет использоваться, будет использоваться только переменная, с которой придется работать.
defined($a) // Вовращает правду если константа была определена.
// Динамическая константа
$dyn_name = "Dynamic_Constant"; // Объявляем переменную, которая в дальнейшем станет именем динамической константы
$dyn_val = 145; // Объявляем переменную, которая в дальнейшем будет значением динамической константы
...
**************************  if-else УСЛОВНАЯ КОНСТРУКЦИЯ = инструкция  *1100258*
...
// Альтернативный код, можно использовать при встраивании php в html. Двоеточие используем в обязательном порядке после if else.
/*  <?php if (isset($_REQUEST['go'])):?>
  ...
  <?php endif?> // обязательно endif в конце  */
**************************  ЦИКЛЫ - конструкции циклов  **********1235698741****
--------------------------  while  ---------------------------------------------
...
} // Тоже имеет альтернативный синтаксис while (): ... endwhile;
while ($a):
  echo "$a<br>";
  $a--;
endwhile;
...
--------------------------  break - continue  ----------------2569845723--------
// Обрывает цикл, либо продолжает цикл с начала. В скобках цифрой можно добавить какой цикл подразумевается. Где 1 текущей в котором используется функция
...
$vegetables = array('corn' => 'yellow', 'beet' => 'red', 'carrot' => 'orange'); // Ассоциативный массив + аналог
$vegetables = ['corn' => 'yellow', 'beet' => 'red', 'carrot' => 'orange']; // Сокращенная новая версия записи массивов используя []
$dinner = array(0 => 'corn and 1', 1 => 'beet and 2', 'admin' => 'corn and 3', 2600 => 'Atari'); // создаем числовой массив + смесь
$dinner = ["Овсянка", "Колбаса", "Суп"]; // Числовой массив можно создать сокращенной записью. Числовой - когда в ключе нумирация цифрами идет. Ключ инкременируется начиная с нуля.
...
$dinner[] = "Вермишель"; // Добавит в конец новое значение
echo $dinner[3]; // Вермишель
count($array) // определение размера массива. Пустой массив передаст 0.
echo "Размер массива - " . count($dinner) . " элемента<br>"; // 4
...
// Объект класса (переменная класса по другому) имеет члены: - свойства (атрибут, поле класса, по сути это таже переменная только с другим названием) - значения различных типов, и - методы - функции, операции, которые могут быть проведены с набором свойств. Эти члены относятся к объекту, хотя создаются в классе.
...
--------------------------  МЕТОД  ---------------------------------------------
// Имя метода принято начинать с мальенькой буквы, а последующие слова с большой, слитно. Модификаторы не обязательны, по умолчанию - публичный, но лучше писать public. Так же не рекомендуется давать такое же название как у класса. Чтобы не писать каждый раз вывод на экран значений свойств при создании нового объекта, можно и нужно этот вывод занести в метод:
class Car
{
  public $color;
  public $wheels = 4;
...
$car1 = new Car();
$car1->color = 'черный';
// $this - эта переменная всегда существует внутри методов. Она помогает добраться до членов класса. Если в индексном файле мы писали сначала с каким объектом работаем $car1->color (перед стрелкой), то в методе мы через $this пишем, что работаем со свойством текущего класса, точнее со свойством текущего объекта в конечном виде. Т.е. если нужно обратиться к свойству или методу внутри класса, то используем this. Теперь вместо длинного кода в индексном файле мы пишем более короткий вызов:
echo $car1->getCarInfo(); // Теперь мы обращаемся не к свойсту, а к методу, метод в свою очередь обращается к свойствам.
// Анахронизм 4-й версии. Когда не было конструктора, создавали метод с тем же названием что и класс. Если нет конструктора в классе, то при создании метода с аналогичным классу именем, этот метод становится конструктором, но выводится замечание. Лучше не использовать, в дальнейшем это удалится.
// Если значение возвращается, называется геттер, если значение устанавливается, называется сеттер.
--------------------------  КОНСТРУКТОР, МАГИЧЕСКИЕ МЕТОДЫ  ------576987--------
// МАГИЧЕСКИЕ МЕТОДЫ - Методы которые вызываются и выполняются автоматически при создании класса, не явно (конструктор, деструктор....)
// Начинается с двойного подчеркивания __construct()
// Автоматизация ввода значений в свойства объекта класса - __construct(). Это та же функция, в которой мы прописываем в скобках переменные. В нем тоже можно вносить значения по умолчанию. В эти переменные в скобках попадут значения, которые ты введешь в скобки при создании объекта. Магия, или магический метод. Магические методы рекомендуется писать перед своими методами.
...
function __destruct() {}
--------------------------  __toString()  --------------------------------------
public function __toString() // Если использовать этот метод в классе, то...
{
  return 'Hello!';
}
echo $book; // При вызове таким образом отобразится то что в теле метода.
// В тело так же можно вернуть любой другой метод, который так же будет отрабатывать.
...
// У свойств и методов одни и теже модификаторы имеют одно и тоже значение.
// Для констант тоже можно использовать модификаторы с 7,1
...
// Советует побольше делать свойства приватными, методы в основном публичными (только служебные приват) - но он ставит либо паб, либо прот.
--------------------------  СТАТИЧЕСКИЕ МЕТОДЫ И СВОЙСТВА  ---------863495------
...
// $this не доступен в статике, т.е. в статическом методе можно работать только черзе self, нет доступа к данным объекта
--------------------------  КОНСТАНТА  -----------------------------------------
...
--------------------------  АБСТРАКТНЫЙ КЛАСС - МЕТОД  --------86245------------
// Задача этого класса запретить создавать объект этого класса. Это логично, если создается базовый класс, шаблон, члены которого будут наследоваться. Но наследовать класс можно.
...
// Если в интерфейсе есть метод, его тут же нужно реализовывать в классе, где подключается интерфейс, как абстрактный метод.
// Константу определенную в интерфейсе нельзя переопределить в дочерних классах, в отличии от константы класса.
// Интерфейсы удобно применять вместе с контролем типов. Как вариант использовать интерфейс для постановки техзадания.
instanceof // Выводит правду или ложь - var_dump($a instanceof A); - является ли объект объектом класса А. Но при этом если у класса А есть родитель, то объект $a будет принадлежать и к класу родителя, и эта функция покажет тру на оба класса. Причем можно проверять и с интерфейсом, если интерфейс подключен к класу, то тоже выдаст тру.
function offerCase(IGadget $product) {} // В данном методе класа мы укзали, что $product должно относиться к интерфейсу IGadget. Благодаря интерфейсу есть контроль к какому объект относится, тут будет работать функция, если передается объект, относящийся к IGadget. По скольку к ноутбуку подключили интерфейс, то он сейчас принадлежит и к ноутбуку и к интерфейсу. А книга не принадлежит к этому интерфейсу. Значит при попытке вставить в этот метод объект книги, будет ошибка.
...
--------------------------  АВТОЗАГРУЗКА  --------------------------------------
// spl_autoload_register(''); // Функция для автозагрузки подключения классов
function autoloder($class) { // Формируем функцию для подключения классов
	$file = __DIR__ . "/classes/{$class}.php"; // Прописывается путь. Ниже посмотри, нужно еще поменять слэш для Линукс.
	if (file_exists($file)) {
		require_once $file;
	}
}
spl_autoload_register('autoloder'); // Таким образом можно под каждую папку делать отдельно это сочетание функций. Но если очень много папок, лучше использовать пространство имен
--------------------------  ПРОСТРАНСТВО ИМЕН  ---------------------------------
// namespace classes\interfaces; - прописывается перед началом класса, а после принято прописывать начиная от корня, но корень не пишется, а пишутся дальше папки по порядку.
// После этого в коде при каждом обращении к объекту нужно либо дописывать путь \classes\BookProduct либо использовать в начале кода:
// use classes\BookProduct;
// В семерке появилась возможность группировать классы из одной папки через запятую - use classes\{BookProduct, NotebookProduct};
// Автозагрузчик при испльзовании пространства имен:

namespace classes;
function autoloder($class) {
	$class = str_replace("\\", '/', $class); // Меняем слэш для Линкус. Винда понимает любой, Линукс только прямой.
	$file = __DIR__ . "/{$class}.php"; // Убрали папку class, она в пространстве прописывается
	if (file_exists($file)) {
		require_once $file;
	}
}
...
--------------------------  ТРЕЙТ  ---------------------------------------------
// Это тоже класс, но от него нельзя создать объект. Напоминает интерфейс, но трейты в отличии могут иметь реализацию. Не меняет тип объекта, но меняет структуру класса.
// Тоже создается в отдельной папке - traits
// trait TColor {} - объявление
--------------------------  ПОЗДНЕЕ СТАТИЧЕСКОЕ СВЯЗЫВАНИЕ  --------------------
// Допустим объявили 2 класса а и б
class A
{
  protected const TEST = "Class A";
public function getTest() // Вывод константы
  {
    var_dump(static::TEST); // Если бы тут стояло self то при выводе вывелась бы констатнта первого класса
  }
}
class B extends A // Наследуется, в том числе вывод константы
{
  const TEST = 'Class B';
}
// Вывод
$a->getTest(); // Class A
$b->getTest(); // Class B - поэтому используем не self а static
--------------------------  ЦЕПОЧКА МЕТОДОВ  -----------------------------------
$book->doAction1()->doAction2(); // Допустим запуск сразу нескольких методов, но метод должен возвращать объект, т.е. обязательно в методе нужно дописать - return $this;
...
exec() // Функция для работы с таблицами бд. Она возвращает сколько было затронуто строк. Это можно использовать для того чтобы выводить в сообщении о кол-ве ввода, изменении и тд.
$query = $pdo->exec("DELETE FROM tbl WHERE id = 2");
echo "Удалена " . $query . "строка"; // Удалена 1 строка
...
fetch() // Вовращает из базы строки. Когда заканчиватся, вовращает ложь. Возвращает по сути два массива и цифровой и ассоциативный, поэтому в скобки есть атрибуты для удобства использования метода:
$a = $q->fetch(PDO::FETCH_ASSOC); // В массиве останется только ассоциативный
// Чтобы извлечь одну запись
$q = $pdo->query("SELECT * FROM catalogs WHERE name LIKE 'Память'"); // выбирается запись с указанными параметрами
...
NOT // Логический оператор отрицания - возвращает истину для ложного и наоборот
XOR // Исключающее или
BETWEEN // Выборка в диапазоне от и до
SELECT * FROM catalogs WHERE id BETWEEN 8 AND 10
IN // Выберет список перечисленный
SELECT * FROM catalogs WHERE id IN (8,9)
NOT IN // Все кроме этого списка. Конструкция возвращает 1, если значение не входит в список, и 0 если входит
SELECT * FROM catalogs WHERE name = 'Процессоры' // Можно извлекать не только числовые значения
LIKE // Услованая выборка. Возвращает 1 (иситну) если есть то что ищем или 0 (ложь) если нет значения. Оператор соответствует оператору равенства, за исключением того, что тут еще есть возможность использовать спецсимволы: % - соответствует любому кол-ву символов либо их отсутствию, _ - соответствует ровно одному символу
SELECT * FROM catalogs WHERE name LIKE '%ь' // Извлечет только строку, в которой значение столбца name оканчивается на мягкий знак, процент говорит, что до мягкого знака могут быть любые символы
SELECT * FROM catalogs WHERE name LIKE '_амять' // В данном случае извлечет строку с любой первой буквой, но с данным окончанием
AS // присваивание нового имени столбцу, можно использовать если длинные имена
SELECT catalog_id, DATE_FORMAT(putdate, '%d.%m.%Y') AS printdate FROM catalogs; // функции DATE_FORMAT() присваивается новый псивдоним printdate

// Не записал 689-693 - Сортировка записей
...
// Еще вывод одного конкретного атрибута
$product->Товар[$i]->attributes()->{'Код'}

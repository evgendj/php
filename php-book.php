...
--------------------------  КЛАСС  ---------------------------------------------
// В имени принято использовать заглавную букву, если из двух слов состоит, то слитно и второе слово тоже с заглавной. Можно начинать с подчеркивания либо буквы, внутри можно использовать и подчеркивание и цифры.
// Есть определенные именна, которые зарезервированы
// Класс можно рассматривать как тип определенной переменной.
// Объект класса (переменная класса по другому) имеет члены:
// - свойства - значения различных типов,
// - методы - функции, операции, которые могут быть проведены с набором свойств.
...
// Класс создается отдельно, а объект создается отдельно, как в функции - описание функции и ее вызов разне вещи.
// Рекомендация 1 класс = 1 файл
...
// $this - эта переменная всегда существует внутри методов. Она помогает добраться до членов класса.
$obj = new MathComplex; // Создание нового объекта класса MathComplex. Теперь объект $obj хранит в себе все данные класса (значения свойств re и im).
...
// Динамическое добавление свойства. Новое свойство объекту класса можно добавить на лету. Но это не рекомендуется. Только в некоторых случаях. Поскольку класс не предполагает добавление новых свойств и нет методов для работы с ними. Кроме этого можно ошибиться в написании имени свойства и задать два свойства вместо одного (color - colour)
$obj->pe = 200;
...
// public - синоним из 4-й версии var - не используй, он устарел.
...
// В аргументы можно передвать переменные, тогда функция будет работать с копией переменной. Но если в аргумент поставить амперсанд, то функция уже работает со значением и может менять в теле исходное значение.
...
func_num_args($i) // Вовращает общее число аргументов при вызове функции

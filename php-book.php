...
// Переменная хранит не сам объект, а ссылку на объект.
--------------------------  ТИПЫ  ----------------------------------------------
// PHP логически понимает переменные которые вводятся. Но тип можно указывать явно.
- integer, int // целое число. Выяснить максимальный размер допустимого числа можно по функции
// echo PHP_INT_MAX; // 9223372036854775807 (у 64 битной версии) - при выходе за этот диапазан преобразуется в след. тип.
- double, float, real // Вещественное число, число с плавающей точкой. Обычная форма записи 346.1256.
...
// Преобразование и установка типа - settype($var, $type), floatval($var), strval($var), intval($var[ , $.base])
...
$a = 10; // Присвоение переменной значения.
- isset() // Проверка существования переменной. Вовр. ложь или истину.
- unset() // Уничтожение переменной, полное. Этим же оператором можно уничтожить элементы массива, удаляется и ключ.
// \n - разрыв строки (пробел), но используем в двойных кавычках, в одинарных не работает.
// Составные операции. Присваивание += чтобы переменную не писать два раза.
$price2 = 6;
echo $price2;
echo "\n";
echo $price2 += 3;
...
// Составное сцепление
$first_mail = 'info';
$second_mail = '@guzun.ru';
echo $first_mail .= $second_mail;
echo '<hr>';
// Если не понятно где заканчивается название переменной, нужно ввести в фигурные скобки.
$preparation = 'Braise';
$meat = 'Beef';
echo "{$preparation}d $meat width...";
echo '<hr>';


// declare(strict_types=l); не понял - отключает фишку по интерпретации типов переменных.
...
// Арифметика. Определение старшенства при больших уровнениях http://www.php.net/language.operators.precedence
// Плюс минус
echo 5+6-2;
print '<hr>';
// Деление умножение
...
echo 10 * 3;
print '<hr>';
// Возведение в степень. Сначала число которое нужно возвести, затем степень после двух звездочек.
echo 4**2;
print '<hr>';
// Деление по модулю / деление без остатка (в математике - кратность)
echo 78 % 33;
echo '<hr>';

// Встраиваемые документы - Когда нужно вывести большой блок на экран. Ставится <<<MENU - три знака меньше и ограничитель, ниже код, затем еще раз ограничитель. В конце ограничитель только с нов. строки, может быть ; если нужно закончить функцию. Если заковычить первый ограничитель (только ограничитель), то переменные не будет выводить (актуальный документ).
$simbol = "руб.";
echo <<<TABLE
<table>
<tr>
...
echo "<hr>";
// foreach ($array as $key => $value) {} - перебор массива, по сути это цикл, который при каждой итерации записывает в ключ и значение данные массива. Для обозначения ключа и значения допускаются какие угодно имена переменных. Но если до упортреблялись, то перезапишутся значениями из массива. В этом цикле можно получать значение каждого элемента массива. Порядок вывода числового массива будет таким же, каким его вводили, скажем если ключи были 0, 1, 3, 2, то и выводить будет криво. Если нужен точный вывод, лучше использовать цикл for(), но при условии кончено что при создании ввода учитывалось соответствие нумирации массива значению.
// чередование классов
...
// чередование калссов при помощи операции по модулю
...
// implode('разделитель', $array) - склеивает элеметы в символьную строку используя разделитель из одинарных кавычек. Можно в кавычки не ставить разделитель, тогда элементы слипнуться.
...
// explode() - разбивает символьную строку на элементы массива.
...
// Принятие решиний. Условный оператор if else.
$proverim = 1;
if ($proverim) {
...
// Сравнения: == равно, != не равно, > больше, < меньше, <= меньше или равно, >= больше или равно. Не путать == (сравнение - проверяется на равенство и возвращает true or false) с = (присваивание), чтобы не путаться, можно число ставить на первое место, тогда не будет перезаписана переменная, а php выдаст ошибку при одном =.
...
// Отрицание истинного значения - ! - восклицательный знак перед выражением сравнивает выражение с false и возвращает true, если выражение не истинно. И false если выражение истинно.
if ($finished == false) {
...
// Циклы - повторное выполнение операций.
// while ()
...
echo '<hr>';
// for ()
for ($n = 1; $n <= 5; $n++) { // В скобках до первой ; инициализируются переменные - инициализирующее выражение, второе выражение проверочное, и третье выражение итерационное - выполняется после каждого шага цикла. В иницилизирущем и итерационном выражениях можно добавлеть через запятую несколько выражений.
	echo "<p>$n</p>";
}

****************** ФУНКЦИИ ***********************

// Инициализровать функцию можно в любом месте кода, интерпритатор читает страницу целиком и выводит функцию вне зависимости от ее размещения.
function say_hallo() { // Создание функции
	echo "<p>Привет, Мир!</p>";
}
say_hallo (); // Вывод функции
// Аргументы можно передавать внутрь функции при вызове функции. Аргумент становится обязательным при вызове функции, если нет аргумента по умолчанию.
function page_header($color) {
	echo "Цвет фона - $color<br>";
}
page_header('ffffff (белый)');
// Аргумент устанавливаемые по умолчанию. Поумолчанию нельзя вставлять переменные, только литералы прописанные внутри функции. Можно через запятую выводить несколько аргументов. Если какие-то стоят по умолчанию, то их нужно использовать в конце аргументов.
function page_header2($title, $text = 'Сегодня поговорим о аргументах функции.', $color2 = 'ffffff') {
	echo "<h1>$title</h1>";
	echo "<p>$text</p>";
	echo "Цвет фона - $color2<br>";
}
page_header2('Аргументы');
echo "<hr>";
page_header2('Аргументы, часть 2', 'Аргументов может быть несколько');
echo "<hr>";
page_header2('Аргументы, часть 3', 'Аргументы могут устанавливаться по умолчанию', '888888');
echo "<hr>";
// Возврат значения функцией. Возвратов можно ставить несколько через условия. Также одну функцию можно использовать в другой функции.
...
echo "Выводим переменную - $m<br>";
summ(50);
echo "<hr>";
// $GLOBALS['m'] Автоглобальная переменная, этот вариант более предпочтителен, с ним код читается лучше. Также внутри фнукции можно изменять глобальный массив.
function summ_2($m) {
...
require 'file.php';
// include 'file.php'; - точно также подключает документ, НО!!! Если возникнут трудности при загрузке, программа не остановится, а просто пропустит подключаемый файл.
include 'file.php';
...
// strlen() - подсчитывает кол-во симоволов в строке
$_post = ' 4569815 ';
...
// str_replace(a,b,c) - замена символов а на символы из переменной b в переменной с. Фигурные скобки я так понял для удобства, работает и без них.
...
// substr(a,b,c) - обрезка символьной строки по кол-ви байт. У кириллицы на одну букву выводится больше одного байта и какой-то вопрос в конце выводится
